...
export const uploadFile = (file, email) => async dispatch => {
  ...
  const config = {
    ...
    onUploadProgress: function(progressEvent) {
      ...
      dispatch({ type: PROGRESS_BAR, payload: percentCompleted });
    },
  ...
  try {
    ...
    if (response.data.success) {
      ...
      dispatch({ type: UPLOAD_FILE_FAIL, payload: response.data.message });
    }
  } catch (error) {
    dispatch(setAlert('Error uploading', 'danger'));
  }
...
export const sendEmail = (name, to, message, link) => async dispatch => {
  const body = JSON.stringify({ name, to, message, link });
  const config = {
    headers: {
...
export const getUserUploads = email => async dispatch => {
  try {
    ...
    if (response.data.success) {
      dispatch({ type: GET_USER_UPLOADS_SUCCESS, payload: response.data });
    } else {
      dispatch({ type: GET_USER_UPLOADS_FAIL });
    }
  } catch (error) {
    dispatch({ type: GET_USER_UPLOADS_FAIL });
  }
